...
def _iterative_forecast_supervised(
    ...
    log_target=True, season_period=3, is_ratio=False
):
    ...
    Xtr, ytr = _make_lag_features(
        y_hist, q_hist, n_lags=n_lags,
        use_time_poly=use_time_poly, use_season=use_season, season_period=season_period
    )
...
def forecast_timeseries(y, q=None, horizon=6, model_choice="Auto (min error)", season_period=3, is_ratio=False):
    ...
    def hgbr_lag(y_hist, q_hist, h):
        ...
        return _iterative_forecast_supervised(mdl, y_hist, q_hist, h, n_lags=None, use_time_poly=True,
            use_season=(q_hist is not None), log_target=True, season_period=season_period, is_ratio=is_ratio)

    def neural_mlp_lag(y_hist, q_hist, h):
        mdl = make_pipeline(StandardScaler(), MLPRegressor(hidden_layer_sizes=(48, 24),
            activation="relu", solver="adam", max_iter=7000, random_state=42))
        return _iterative_forecast_supervised(mdl, y_hist, q_hist, h, n_lags=None, use_time_poly=True,
            use_season=(q_hist is not None), log_target=True, season_period=season_period, is_ratio=is_ratio)
    ...
    chosen_fn = models[chosen]
    y_future = chosen_fn(y, None if q is None else np.asarray(q, dtype=int), horizon)
    ...
    return y_future, chosen, scores, chosen_fn
...
# ============================================================
# PREDICTIVE RISK MODEL — HELPER FUNCTIONS
# ============================================================
RISK_CONDITIONS = [
    ("FB Ratio",                        "lt",  0.05,    3, "Fund balance ratio is critically low (< 5%)"),
    ("FB Ratio",                        "lt",  0.10,    2, "Fund balance ratio is below the 10% target"),
    ("Liabilities to Assets",           "gt",  0.90,    3, "Liabilities-to-assets ratio is critically elevated (> 0.90)"),
    ("Liabilities to Assets",           "gt",  0.75,    2, "Liabilities-to-assets ratio is above the moderate threshold (> 0.75)"),
    ("Current Ratio",                   "lt",  1.00,    3, "Current ratio is below 1.0 — bills may not be payable"),
    ("Current Ratio",                   "lt",  1.50,    2, "Current ratio is below the 1.50 best-practice floor"),
    ("Unrestricted Days COH",           "lt",  40.0,    3, "Cash on hand is critically low (< 40 days)"),
    ("Unrestricted Days COH",           "lt",  60.0,    2, "Cash on hand is below the 60-day best-practice threshold"),
    ("Unrestricted Cash & Equivalents", "lt",  500_000, 2, "Unrestricted cash is below $500,000"),
]
...
def _score_row(row: pd.Series) -> int:
    ...
    exp = row.get("Total Expenses", np.nan)
...
def _build_risk_dataset(df_src: pd.DataFrame) -> pd.DataFrame:
    ...
    for school, grp in df_feat.groupby("Schools", sort=False):
        ...
        for idx2 in range(len(grp)):
            targets.append({"Schools": school, "Fiscal Year": grp.loc[idx2, "Fiscal Year"],
                            "_next_q_risk": shifted.iloc[idx2]})
    df_targets = pd.DataFrame(targets)
    df_feat = pd.merge(df_feat, df_targets, on=["Schools", "Fiscal Year"], how="left")
    return df_feat
...
def _train_risk_model(df_feat: pd.DataFrame):
    ...
    if df_train.empty:
        return None, [], []
    exclude = {"Schools", "Fiscal Year", "_sort_key", "_risk_label", "_risk_score", "_next_q_risk", "_expense_over_revenue"}
    ...
    clf = RandomForestClassifier(n_estimators=300, max_depth=6, min_samples_leaf=2, random_state=42, class_weight="balanced")
    clf.fit(X, y_labels)
...
def _explain_risk(row: pd.Series, feat_cols_present: list) -> list:
    ...
    exp = row.get("Total Expenses", np.nan)
    if pd.notna(rev) and pd.notna(exp) and float(exp) > float(rev):
        gap = float(exp) - float(rev)
        explanations.append(f"Total expenses exceed revenue by ${gap:,.0f}")
    qoq_checks = [
    ...
    for col, direction, msg in qoq_checks:
        chg_col = f"{col}_chg"
        if chg_col not in row.index:
            continue
        chg = row.get(chg_col, np.nan)
        if pd.isna(chg):
            continue
...
# ============================================================
# EARLY WARNING SYSTEM — HELPER FUNCTIONS  (NEW)
# ============================================================

# Warning rule definitions: (column, operator, threshold, severity, message)
EWS_RULES = [
    ("FB Ratio",                        "lt",  0.05,       "🔴 Severe",   "Fund Balance Ratio critically low (< 5%)"),
    ("FB Ratio",                        "lt",  0.10,       "🟡 Moderate", "Fund Balance Ratio below 10% target"),
    ("Liabilities to Assets",           "gt",  0.90,       "🔴 Severe",   "Liabilities-to-Assets critically high (> 0.90)"),
    ("Liabilities to Assets",           "gt",  0.75,       "🟡 Moderate", "Liabilities-to-Assets elevated (> 0.75)"),
    ("Current Ratio",                   "lt",  1.00,       "🔴 Severe",   "Current Ratio below 1.0 — may not cover short-term bills"),
    ("Current Ratio",                   "lt",  1.50,       "🟡 Moderate", "Current Ratio below 1.50 best-practice floor"),
    ("Unrestricted Days COH",           "lt",  40.0,       "🔴 Severe",   "Cash on Hand critically low (< 40 days)"),
    ("Unrestricted Days COH",           "lt",  60.0,       "🟡 Moderate", "Cash on Hand below 60-day best-practice threshold"),
    ("Unrestricted Cash & Equivalents", "lt",  500_000,    "🟡 Moderate", "Unrestricted Cash below $500,000"),
]

def _ews_get_latest_snapshot(df_src: pd.DataFrame) -> pd.DataFrame:
    """
    Return one row per school: the latest quarter's values for all EWS columns.
    Uses the existing sort_fy logic. Works on a copy; does not modify df.
    """
    df_work = df_src.copy()
    df_work["_sk"] = df_work["Fiscal Year"].apply(sort_fy)

    # Coerce numeric columns used in EWS rules
    ews_cols = list({r[0] for r in EWS_RULES} | {"Total Revenue", "Total Expenses"})
    for c in ews_cols:
        if c in df_work.columns:
            df_work[c] = pd.to_numeric(df_work[c], errors="coerce")

    # Keep latest quarter per school
    latest = (
        df_work.sort_values("_sk")
        .groupby("Schools", sort=False)
        .last()
        .reset_index()
    )
    return latest
...
def _ews_evaluate(latest_df: pd.DataFrame) -> pd.DataFrame:
    """
    Evaluate every EWS rule against the latest-quarter snapshot.
    Returns a DataFrame with columns: School, Quarter, Severity, Warning.
    Structural deficit (Expenses > Revenue) is also evaluated.
    """
    records = []
    for _, row in latest_df.iterrows():
        school  = row.get("Schools", "")
        quarter = row.get("Fiscal Year", "")

        # Rule-based warnings
        for col, op, thr, severity, msg in EWS_RULES:
            if col not in row.index:
                continue
            v = row.get(col, np.nan)
            if pd.isna(v):
        ...
        if pd.notna(rev) and pd.notna(exp) and float(exp) > float(rev):
            ...
            records.append({
                ...
                "Quarter":  quarter,
                "Severity": "🔴 Severe",
                "Warning":  f"Structural deficit — expenses exceed revenue by ${gap:,.0f}",
            })

    return pd.DataFrame(records, columns=["School", "Quarter", "Severity", "Warning"])


# ============================================================
# CFO AI ASSISTANT — HELPER FUNCTIONS  (NEW)
# ============================================================

def _cfo_get_latest(df_src: pd.DataFrame) -> pd.DataFrame:
    """
    Return one row per school representing the latest available quarter.
    Coerces all key columns to numeric. Does not modify the original df.
    """
    df_work = df_src.copy()
    df_work["_sk"] = df_work["Fiscal Year"].apply(sort_fy)

    cfo_cols = [
        "FB Ratio", "Liabilities to Assets", "Current Ratio",
...
def _cfo_fmt_row(row: pd.Series) -> dict:
    """Format a single school row into a display-friendly dict."""
    def _fv(col, fmt):
        v = row.get(col, np.nan)
        if pd.isna(v):
            return "—"
    ...
    return {
        "School":              row.get("Schools", ""),
        "Quarter":             row.get("Fiscal Year", ""),
        "FB Ratio":            _fv("FB Ratio", "{:.1%}"),
        "Current Ratio":       _fv("Current Ratio", "{:.2f}"),
        "Days COH":            _fv("Unrestricted Days COH", "{:,.0f}"),
        "Liab / Assets":       _fv("Liabilities to Assets", "{:.2f}"),
        "Unrest. Cash":        _fv("Unrestricted Cash & Equivalents", "${:,.0f}"),
        "Total Revenue":       _fv("Total Revenue", "${:,.0f}"),
        "Total Expenses":      _fv("Total Expenses", "${:,.0f}"),
    }


def _cfo_answer(question: str, latest_df: pd.DataFrame) -> tuple:
    """
    Keyword-matching rules engine.
    Returns (answer_title: str, result_df: pd.DataFrame | None, matched: bool).
    """
    q = question.lower().strip()

    # ── 1. Liquidity risk ────────────────────────────────────────────────────
    if any(kw in q for kw in ["liquidity", "liquid", "current ratio", "days cash", "days coh", "cash on hand"]):
        mask = pd.Series([False] * len(latest_df), index=latest_df.index)
        if "Current Ratio" in latest_df.columns:
            cr = pd.to_numeric(latest_df["Current Ratio"], errors="coerce")
            mask |= cr < 1.5
        if "Unrestricted Days COH" in latest_df.columns:
            dch = pd.to_numeric(latest_df["Unrestricted Days COH"], errors="coerce")
            mask |= dch < 60
        subset = latest_df[mask]
        rows = [_cfo_fmt_row(r) for _, r in subset.iterrows()]
        title = f"Schools with potential **liquidity risk** (Current Ratio < 1.50 or Days COH < 60) — {len(rows)} found"
        return title, pd.DataFrame(rows) if rows else None, True

    # ── 2. Fund balance weakness ─────────────────────────────────────────────
    if any(kw in q for kw in ["fund balance", "fb ratio", "low fund", "reserve", "fund ratio"]):
        if "FB Ratio" in latest_df.columns:
            fb = pd.to_numeric(latest_df["FB Ratio"], errors="coerce")
            subset = latest_df[fb < 0.10]
            rows = [_cfo_fmt_row(r) for _, r in subset.iterrows()]
            title = f"Schools with **low fund balance** (FB Ratio < 10%) — {len(rows)} found"
            return title, pd.DataFrame(rows) if rows else None, True
        return "Column 'FB Ratio' not found in data.", None, True

    # ── 3. High leverage / liabilities ───────────────────────────────────────
    if any(kw in q for kw in ["liabilit", "leverage", "debt", "high liabilit", "liab"]):
        if "Liabilities to Assets" in latest_df.columns:
            la = pd.to_numeric(latest_df["Liabilities to Assets"], errors="coerce")
            subset = latest_df[la > 0.75]
            rows = [_cfo_fmt_row(r) for _, r in subset.iterrows()]
            title = f"Schools with **high liabilities** (Liabilities-to-Assets > 0.75) — {len(rows)} found"
            return title, pd.DataFrame(rows) if rows else None, True
        return "Column 'Liabilities to Assets' not found in data.", None, True

    # ── 4. Structural deficit ────────────────────────────────────────────────
    if any(kw in q for kw in ["deficit", "spending more", "expenses more", "expense more",
                               "spending exceed", "revenue", "overspend"]):
        if "Total Revenue" in latest_df.columns and "Total Expenses" in latest_df.columns:
            rev = pd.to_numeric(latest_df["Total Revenue"], errors="coerce")
            exp = pd.to_numeric(latest_df["Total Expenses"], errors="coerce")
            subset = latest_df[exp > rev]
            rows = [_cfo_fmt_row(r) for _, r in subset.iterrows()]
            title = f"Schools where **expenses exceed revenue** (structural deficit) — {len(rows)} found"
            return title, pd.DataFrame(rows) if rows else None, True
        return "Columns 'Total Revenue' / 'Total Expenses' not found in data.", None, True

    # ── 5. All schools summary ───────────────────────────────────────────────
    if any(kw in q for kw in ["all school", "list school", "show school", "every school", "summary"]):
        rows = [_cfo_fmt_row(r) for _, r in latest_df.iterrows()]
        title = f"Latest quarter snapshot for **all {len(rows)} schools**"
        return title, pd.DataFrame(rows) if rows else None, True
...
def make_gauge(value, vmin, vmax, threshold, good_direction, value_fmt, title_text):
    if np.isfinite(value):
        vmax_eff = max(vmax, float(value) * 1.10)
    else:
        vmax_eff = vmax
...
if metric_group == "Executive Summary (School Report Card)":
    ...
    with c1:
        ...
        fig_fb = make_gauge(value=fb, vmin=0.0, vmax=0.60, threshold=0.10, good_direction="gte", value_fmt=".0%", title_text="Fund Balance Ratio")
    ...
    with c2:
        ...
        fig_la = make_gauge(value=la, vmin=0.0, vmax=1.50, threshold=0.90, good_direction="lte", value_fmt=".2f", title_text="Liabilities to Assets")
    ...
    with c3:
        ...
        fig_cr = make_gauge(value=cr, vmin=0.0, vmax=3.0 if not np.isfinite(cr) else max(3.0, cr*1.1), threshold=1.50, good_direction="gte", value_fmt=".2f", title_text="Current Ratio")
        st.plotly_chart(fig_cr, use_container_width=True)
    with c4:
        st.markdown("**Unrestricted Cash on Hand: Enough cash to pay bills for 60+ days if $0 incoming cash?**  \n_Unrestricted Cash / ((Total Exp. − Depreciation)/365).  •  Best practice ≥ 60_")
        vmax_dch = 300.0 if not np.isfinite(dch) else max(300.0, math.ceil(dch/50.0)*50.0)
        fig_dch = make_gauge(value=dch, vmin=0.0, vmax=vmax_dch, threshold=60.0, good_direction="gte", value_fmt=",.0f", title_text="Unrestricted Days COH")
...
""")
    ...
    md_lines = [
        f"# Executive Summary — {school_name}", f"**Overall Health:** {overall}", "",
        "## Current Status",
        ...
        "## Thresholds",
        "* FB Ratio ≥ 10%", "* Current Ratio ≥ 1.50×", "* Unrestricted Days COH ≥ 60", "* Liabilities to Assets ≤ 0.90",
    ]
    st.download_button(
...
# ============================================================
    ...
    forecast_mode    = st.sidebar.radio("🧊 Forecast Mode", ["Freeze at selected quarter", "Unfrozen (use all actuals)"], index=0)
    ...
    csaf_model_choice = st.sidebar.selectbox("🧠 CSAF Forecast Model:", ["Auto (min error)"] + [
        "Ensemble (Seasonal Naive + Drift + Robust Seasonal + Trend×Seasonality)",
        "Seasonal Naive + Drift (recommended baseline)", "Seasonal Naive (same quarter last year)",
        "Robust Seasonal Regression (Huber + quarter dummies, log1p)",
        "Trend × Seasonal Index (linear trend on de-seasonalized)",
        "HGBR Lag + Trend + Season (recommended)", "Neural MLP Lag + Season", "Ridge Lag + Season",
    ], index=0)
    ...
    y_future, chosen_model, scores, chosen_fn = forecast_timeseries(
        y=y, q=q, horizon=horizon_q, model_choice=csaf_model_choice, season_period=3, is_ratio=is_ratio)
    ...
    actual_part   = pd.DataFrame({"Period": actual_full["Fiscal Year"].astype(str),
                                   "Value": actual_vals, "Type": "Actual"}).dropna(subset=["Value"])
    ...
    fig = px.bar(combined, x="Period", y="Value", color="Type", barmode="group",
    ...
    if show_intervals:
        p10, p50, p90 = bootstrap_intervals(y_hist=y, q_hist=q, horizon=horizon_q, model_fn=chosen_fn,
            season_period=3, n_sims=n_sims, p_lo=p_lo, p_hi=p_hi, is_ratio=is_ratio)
        fig.add_trace(go.Scatter(x=future_labels + future_labels[::-1], y=list(p90) + list(p10[::-1]),
            fill="toself", mode="lines", line=dict(width=0), name=f"Interval P{p_lo}–P{p_hi}", showlegend=True, opacity=0.20))
    ...
    fig.update_layout(title=dict(text=f"{selected_school} — {selected_metric}", x=0.01, y=0.985),
...
# ============================================================
    ...
    d = df_budget_long[(df_budget_long["Schools"] == selected_school) &
                        (df_budget_long["Metric"].isin(selected_metrics)) &
                        (df_budget_long["Fiscal Year"].isin(selected_fy))].copy()
    ...
    for met in selected_metrics:
        sub = d[d["Metric"] == met]
        fig.add_trace(go.Bar(x=sub["Fiscal Year"], y=sub["ValueNum"], name=f"{met}",
            marker_color=ENROLL_COLORS.get((met, "Actual"), None),
            text=[fmt_enroll(met, v) for v in sub["ValueNum"]], textposition="outside"))
    fig.update_layout(title=dict(text=f"{selected_school} — Enrollment (Actuals)", x=0.01, y=0.98),
        barmode="group", bargap=BARGAP, bargroupgap=BARGROUPGAP,
...
# ============================================================
    ...
    if need_oct:
        if len(oct_hist_df) < 3:
            ...
            combined_frames.append(pd.DataFrame({
                "FY": oct_hist_df["FY"], "ValueNum": oct_hist_df["ValueNum"],
                "Metric": "October 1 Count", "Type": "Actual"
            }))
            combined_frames.append(pd.DataFrame({
                "FY": future_years, "ValueNum": oct_future,
    ...
    if need_feb:
        if retention is None:
            ...
            model_info_rows.append({
                "Metric": "February 1 Count", "Method": "Derived from October", "YoY % used": "",
                "Notes": f"Feb = Oct × retention, retention={retention:.3f} (clipped {ret_lo:.2f}–{ret_hi:.2f})"
            })
            if not feb_hist_df.empty:
                combined_frames.append(pd.DataFrame({
                    "FY": feb_hist_df["FY"], "ValueNum": feb_hist_df["ValueNum"],
            ...
            if (not has_feb_freeze) and need_oct and (not oct_hist_df.empty):
                ...
                if not oct_freeze_row.empty and np.isfinite(oct_freeze_row["ValueNum"].iloc[0]):
                    ...
                    combined_frames.append(pd.DataFrame([{
                        "FY": freeze_fy_str, "ValueNum": feb_est,
            ...
            if need_oct and (oct_future is not None):
                ...
                if show_intervals:
                    band = 0.015
                    combined_frames.append(pd.DataFrame({
                        "FY": future_years, "ValueNum": feb_future * (1 - band),
                ...
                combined_frames.append(pd.DataFrame({
                    "FY": future_years, "ValueNum": feb_future,
                    "Metric": "February 1 Count", "Type": "Forecast (Frozen)"
                }))
    ...
    for met in selected_metrics:
        ...
        y_future, chosen_model, scores, chosen_fn = forecast_timeseries(
            ...
            season_period=1, is_ratio=is_ratio
        )
        model_info_rows.append({
            "Metric": met, "Method": "Time-series", "YoY % used": "",
            "Notes": f"Model: {chosen_model}"
        })
        combined_frames.append(pd.DataFrame({
            "FY": dh["FY"], "ValueNum": dh["ValueNum"], "Metric": met, "Type": "Actual"
        }))
        combined_frames.append(pd.DataFrame({
            "FY": future_years, "ValueNum": y_future, "Metric": met, "Type": "Forecast (Frozen)"
        }))
...
# ============================================================
# NEW: PREDICTIVE RISK MODEL
# ============================================================
elif metric_group == "Predictive Risk Model":
    st.markdown("## 🤖 Predictive Risk Model — Next-Quarter Financial Risk")
    st.caption(
        "Uses quarter-level lag features from CSAF and balance-sheet metrics "
        "to forecast each school's next-quarter risk tier: **Low / Medium / High**."
    )
    ...
    if clf is None:
        st.error(
            "❌ Not enough historical data to train the risk model. "
            "Need at least 2+ quarters per school."
        )
        st.stop()
 
    # ── Predict for the latest quarter of every school ────────────────────────
    exclude_cols = {
        "Schools", "Fiscal Year", "_sort_key",
        "_risk_label", "_risk_score", "_next_q_risk",
        "_expense_over_revenue",
    }
 
    latest_rows = (
        df_feat.sort_values("_sort_key")
        .groupby("Schools", sort=False)
        .last()
        .reset_index()
    )
    ...
    pred_probas = clf.predict_proba(X_latest)        # shape (n_schools, n_classes)
    ...
    selected_school_rm = st.sidebar.selectbox(
        "🏫 Select School (detail view):",
        sorted(latest_rows["Schools"].tolist())
    )
 
    # ── Pull data for selected school ─────────────────────────────────────────
    ...
    st.markdown(
        ...
        <div style="
            ...
            margin-bottom: 18px;
        ">
          <div style="font-size:13px;text-transform:uppercase;letter-spacing:.08em;
                      color:#485569;font-weight:700;">
            {selected_school_rm} — Forecast based on {current_fy}
          </div>
          <div style="font-size:36px;font-weight:900;color:{risk_color};margin:6px 0 2px 0;">
            {risk_emoji} Next-Quarter Risk: {pred_risk}
          </div>
    ...
    # ── KPI strip ─────────────────────────────────────────────────────────────
    def _kv(col):
        return school_latest_hist.get(col, np.nan)
 
    fb_v  = _kv("FB Ratio")
    cr_v  = _kv("Current Ratio")
    dch_v = _kv("Unrestricted Days COH")
    la_v  = _kv("Liabilities to Assets")
 
    def _metric_tile(col_obj, label, value_str, good: bool):
        color = "#16a34a" if good else "#dc2626"
        col_obj.markdown(
            f"""<div style="border:1px solid {color}33;border-radius:10px;
                           padding:12px 14px;background:{color}0d;">
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:.07em;
                        color:#6b7280;font-weight:700;">{label}</div>
            <div style="font-size:26px;font-weight:800;color:{color};">{value_str}</div>
            </div>""",
            unsafe_allow_html=True
        )
 
    k1, k2, k3, k4 = st.columns(4)
    _metric_tile(k1, "FB Ratio",
                 f"{fb_v:.1%}" if np.isfinite(fb_v) else "—",
                 np.isfinite(fb_v) and fb_v >= 0.10)
    _metric_tile(k2, "Current Ratio",
                 f"{cr_v:.2f}" if np.isfinite(cr_v) else "—",
                 np.isfinite(cr_v) and cr_v >= 1.50)
    _metric_tile(k3, "Days Cash on Hand",
                 f"{dch_v:,.0f}" if np.isfinite(dch_v) else "—",
                 np.isfinite(dch_v) and dch_v >= 60)
    _metric_tile(k4, "Liabilities / Assets",
                 f"{la_v:.2f}" if np.isfinite(la_v) else "—",
                 np.isfinite(la_v) and la_v <= 0.90)
    ...
    if pred_risk == "Low":
        st.success(
            ...
            "School appears financially stable heading into next quarter."
        )
    else:
    ...
    for col in detail_metrics:
        ...
        detail_rows.append({
            "Metric":          col,
            "Current Quarter": _fmt_v(v_curr),
            "Prior Quarter":   _fmt_v(v_lag1),
            "Change":          _fmt_chg(v_chg),
        })
    ...
    # ── All HIGH-risk schools ─────────────────────────────────────────────────
    st.markdown("#### 🔴 All Schools Forecasted HIGH Risk Next Quarter")
    display_cols = [
        "Schools", "Fiscal Year", "Predicted Next-Q Risk", "Confidence",
    ...
    high_df = (
        latest_rows[latest_rows["Predicted Next-Q Risk"] == "High"][avail_display]
        .copy()
    ...
    rank_df = (
        latest_rows[["Schools", "Fiscal Year", "Predicted Next-Q Risk", "Confidence"]]
        .copy()
    ...
    rank_df.index = rank_df.index + 1    # 1-based ranking
    ...
    st.download_button(
        ...
        data=csv_bytes,
        file_name="nola_next_quarter_risk_predictions.csv",
        mime="text/csv"
    )
