...
BASE_HEIGHT_TALL = 880
BASE_HEIGHT_TALLER = 980
CSAF_PRED_HEIGHT = 1050     # bigger CSAF predicted chart
CSAF_BAR_GAP = 0.04         # thicker bars
...
def std_fyq_label(x):
    """standardize the FY token but keep quarter text, e.g. 'FY2022 Q1' → 'FY22 Q1'."""
    s = str(x).strip()
    parts = s.split()
    if not parts:
        return s
...
def best_model_forecast_yearly(y, n_future, is_ratio=False):
    """
    Yearly forecaster (Budget/Enrollment): choose best model by time-series CV on log1p.
    """
    if len(y) == 0:
...
# ============================================================
# CSAF PREDICTED (BAR, BIG + MIN ERROR MODEL SELECTION)
# ============================================================
if metric_group == "CSAF Predicted":
    ...
    def feats_quarter_dummies(q_arr_int):
        Q2 = (q_arr_int == 2).astype(int).reshape(-1, 1)
        Q3 = (q_arr_int == 3).astype(int).reshape(-1, 1)
    ...
    # Features: time + quadratic + seasonal dummies, target: log1p(clip(y))
    y_clip = np.clip(y, 0, None)
    y_log = np.log1p(y_clip)

    t = np.arange(len(y_log)).reshape(-1, 1).astype(float)
    X = np.hstack([t, t**2, feats_quarter_dummies(q)])
    ...
    fig.update_layout(bargap=CSAF_BAR_GAP, bargroupgap=0.02)
    fig = apply_plot_style(fig, CSAF_PRED_HEIGHT)
    st.plotly_chart(fig, use_container_width=True)

# ============================================================
# BUDGET TO ENROLLMENT (ACTUAL) — selector Bar / Line / Combo
#   - NO best practice line (per your instruction)
#   - Line is clean connected straight segments: sorted + connectgaps=True
#   - One chart per school (no facet axis crash)
# ============================================================
    ...
    chart_mode = st.sidebar.selectbox(
        "📈 View Mode:",
        ["Combo (Bar + Line + markers)", "Line (markers)", "Bar"],
        index=0
    )
    ...
    axis_order = [fy for fy in FY22_TO_FY28 if fy in set(selected_fy_b)]
    ...
    for school in selected_schools:
        ...
        if chart_mode in ["Bar", "Combo (Bar + Line + markers)"]:
            for met in df_main["Metric"].unique():
                d = df_main[df_main["Metric"] == met].sort_values("sort_key")
        ...
        # Ratio (optional) on secondary axis if selected
...
# ============================================================
# BUDGET TO ENROLLMENT PREDICTED — selector Bar / Line / Combo
#   - NO best practice line (per your instruction)
#   - Forecast uses minimal-error model selection (CV MAE on log1p)
# ============================================================
    ...
    selected_metrics_b = st.sidebar.multiselect(
        "📌 Choose Metric(s):",
        metrics_all,
        default=["October 1 Count", "February 1 Count"]
    )
    ...
    show_model_info = st.sidebar.checkbox("Show model + error details", value=False)
    ...
    for met in selected_metrics_b:
        # history used for training (<= freeze, within selected training FYs)
        dfh = df_budget_long[
        ...
        y_hist = pd.to_numeric(dfh["Value"], errors="coerce").values.astype(float)
        y_hist = y_hist[~np.isnan(y_hist)]

        is_ratio = (met == "Budget to Enrollment Ratio")

        if n_future_b > 0 and len(y_hist) >= 3:
            y_future, best_name, best_err = best_model_forecast_yearly(y_hist, n_future_b, is_ratio=is_ratio)
        elif n_future_b > 0 and len(y_hist) == 2:
            slope = (y_hist[-1] - y_hist[-2])
            last = y_hist[-1]
            y_future = np.array([max(0.0, last + slope * (i + 1)) for i in range(n_future_b)], dtype=float)
            best_name, best_err = "2pt-trend", np.nan
        elif n_future_b > 0 and len(y_hist) == 1:
            y_future = np.array([max(0.0, y_hist[0]) for _ in range(n_future_b)], dtype=float)
        ...
        actual_part = pd.DataFrame({
            ...
            "ValueNum": pd.to_numeric(actual_now["Value"], errors="coerce"),
            "Metric": met,
            "Type": "Actual"
    ...
    if chart_mode in ["Bar", "Combo (Bar + Line + markers)"]:
        for met in selected_metrics_b:
            ...
            for tname in ["Actual", "Forecast (Frozen)"]:
                dt = dmet[dmet["Type"] == tname].sort_values("sort_key")
                if dt.empty:
                    continue
                fig.add_trace(go.Bar(
                    x=dt["FY"],
                    y=dt["ValueNum"],
                    name=f"{met} — {tname}",
                    opacity=0.88 if tname == "Actual" else 0.55,
                    text=[
                        (f"{v:.0%}" if met == "Budget to Enrollment Ratio" else f"{v:,.0f}")
                        for v in dt["ValueNum"]
                    ],
                    textposition="outside"
                ))
    ...
    fig.update_layout(
        title=f"Budget / Enrollment Prediction: {selected_school_b} (Freeze at {train_through_b})",
        barmode="group",
    )
    fig.update_xaxes(categoryorder="array", categoryarray=axis_order, tickangle=0)
    fig = apply_plot_style(fig, BASE_HEIGHT_TALLER)
...
# ============================================================
# CSAF METRICS (BAR) + BEST PRACTICE
# ============================================================
elif metric_group == "CSAF Metrics":
    st.markdown("## 📌 CSAF Metrics (Actuals)")
    ...
    filtered["TextLabel"] = filtered["ValueNum"].apply(fmt_csaf_text)
    ...
    fig = add_best_practice_csaf(fig, selected_metric)
    fig.update_layout(bargap=0.06, bargroupgap=0.03)  # thicker than default
    fig = apply_plot_style(fig, BASE_HEIGHT_TALL)
    st.plotly_chart(fig, use_container_width=True)
...
# ============================================================
    ...
    filtered["TextLabel"] = [fmt_other(m, v) for m, v in zip(filtered["Metric"], filtered["ValueNum"])]
    ...
    fig.update_layout(bargap=0.06, bargroupgap=0.03)
    fig = apply_plot_style(fig, BASE_HEIGHT_TALL)
    st.plotly_chart(fig, use_container_width=True)
